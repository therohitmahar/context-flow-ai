              ...
              {[
                {
                  ...
                  desc: 'A comprehensive flow for scaffolding React applications with best practices.',
                  tags: ['TypeScript', 'Architecture'],
                  icon: 'account_tree'
                },
                {
                  title: 'Marketing Strategist',
                  desc: 'Generate cohesive multi-channel campaigns from a single product brief.',
                  tags: ['Copywriting', 'Creative'],
                  icon: 'campaign'
                },
                {
                  title: 'Code Security Auditor',
                  desc: 'Audit your logic for vulnerabilities and suggest robust hardening measures.',
                  tags: ['Security', 'Backend'],
                  icon: 'security'
                }
              ].map((template, i) => (
                <div key={i} className="group bg-surface-dark border border-slate-800 rounded-xl overflow-hidden hover:border-primary/50 transition-all duration-300">
                  <div className="h-40 bg-[#151b28] relative p-4 overflow-hidden border-b border-slate-800">
                    <div className="absolute inset-0 flex items-center justify-center opacity-40 group-hover:opacity-60 transition-opacity">
                      <span className="material-symbols-outlined text-6xl text-primary">{template.icon}</span>
                    </div>
                  </div>
                  <div className="p-5">
                    <div className="flex gap-2 mb-3">
                      {template.tags.map(tag => (
                        <span key={tag} className="px-2 py-0.5 rounded-full bg-primary/10 border border-primary/20 text-[10px] font-bold text-primary uppercase tracking-wider">{tag}</span>
                      ))}
                    </div>
                    <h4 className="font-bold text-lg text-white mb-2">{template.title}</h4>
                    <p className="text-slate-500 text-sm mb-4 leading-relaxed">{template.desc}</p>
                    <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                      <span className="text-xs font-semibold text-slate-400 group-hover:text-primary transition-colors flex items-center gap-1">
                        Use Template <span className="material-symbols-outlined text-xs">arrow_forward</span>
                      </span>
                    </div>
                  </div>
                </div>
